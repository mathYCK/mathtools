...
<body>
    ...
    <div class="chat-container">
        <div class="chat-display">
            ...
            <div class="chat-bubble" id="aiBubble">進入高級挑戰！這些題目需要更多耐心，加油！</div>
        </div>
    </div>
    <div class="controls">
        <select id="levelSelect">
            <option value="custom">-- 選擇任務 --</option>
            <option value="t1">1. x² + 8x + 15</option>
            <option value="t2" disabled>2. x² + 10x + 16 (鎖定)</option>
            <option value="t3" disabled>3. x² + 10x + 24 (鎖定)</option>
        </select>
        <div class="btn-group">
    ...
    <script>
        const tasks = {
            't1': { label: "x² + 8x + 15", x2:1, x:8, one:15 },
            't2': { label: "x² + 10x + 16", x2:1, x:10, one:16 },
            't3': { label: "x² + 10x + 24", x2:1, x:10, one:24 }
        };
        ...
        function unlockNext(currentKey) {
            ...
            if(currentKey === 't1') {
                ...
                opt.disabled = false; opt.innerText = "2. x² + 10x + 16";
            } else if(currentKey === 't2') {
                const opt = document.querySelector('option[value="t3"]');
                opt.disabled = false; opt.innerText = "3. x² + 10x + 24";
