...
<body>
    <h1>挑戰任務篇</h1>
    <div class="top-info">
        <div class="progress-bar"><span class="badge active">自由挑戰模式</span></div>
        <div class="rule-banner">⚠️ 規則：紅色(x²)與黃色(1)不可直接相鄰！</div>
    </div>
    <div class="chat-container">
        <div class="chat-display">
            <div class="chat-avatar" onclick="giveHint()">🤖</div>
            <div class="chat-bubble" id="aiBubble">歡迎來到挑戰區！這裡有 2x² 和 3x² 的題目，發揮你的實力吧！</div>
        </div>
    </div>
    <div class="controls">
        <select id="levelSelect">
            <option value="custom">-- 選擇挑戰 --</option>
            <option value="t1">1. 2x² + 5x + 2</option>
            <option value="t2">2. 2x² + 7x + 3</option>
            <option value="t3">3. 3x² + 7x + 2</option>
            <option value="t4">4. 2x² + 11x + 12</option>
            <option value="t5">5. 3x² + 8x + 4</option>
        </select>
        <div class="btn-group">
    ...
    <script>
        const tasks = {
            't1': { label: "2x² + 5x + 2", x2:2, x:5, one:2 },
            't2': { label: "2x² + 7x + 3", x2:2, x:7, one:3 },
            't3': { label: "3x² + 7x + 2", x2:3, x:7, one:2 },
            't4': { label: "2x² + 11x + 12", x2:2, x:11, one:12 },
            't5': { label: "3x² + 8x + 4", x2:3, x:8, one:4 }
        };
